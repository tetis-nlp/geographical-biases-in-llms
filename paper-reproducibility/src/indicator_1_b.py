...
if __name__ == "__main__":
    ...
    df['CountryName'] = df['Country name EN'].str.lower()
        ...
        if list_of_models[model_name]["type"] == "slm":
            ...
            def self_masking(city):
                ...
                if model_name != "camembert": # english
                    masked_sentence = f'The city of {city} is located in the country of  {list_of_models[model_name]["mask"]}.'
                else: # french
            ...
            def self_masking(city):
                messages = [
                    {"role": "user", "content": "Name the country containing Montpellier. Only give the country."},
                    {"role": "assistant", "content": "France"},
                    {"role": "user", "content": f"Name the country containing {city}. Only give the country."}]
            ...
            def self_masking(city):
                ...
                try:
                    ...
                    return "time_out"
        try:
            df[f"predicted_{model_name}"] = df["city"].apply(self_masking).str.lower()
            df[f"correct_{model_name}"] = df['CountryName'] == df[f"predicted_{model_name}"]
            df.to_csv(f"./output/self_masking_city_which_country{model_name}")
    ...
    df_accuracy_model.to_csv('./output/self_masking_city_which_countrymodel_without_pivot.csv')
    df_accuracy_model = df_accuracy_model.pivot(index='model', columns='ContinentName', values='Accuracy')
    df_accuracy_model.to_csv('./output/self_masking_city_which_countrymodel.csv')

    dff = pd.read_csv('./output/self_masking_city_which_countrymodel.csv')
    ...
    dff.to_csv('./output/self_masking_city_which_countrymodel_formated.csv')
    print(dff)
